...
folder_path = r"C:\Users\lenka\OneDrive\Documenten\Afstuderen Master\detectie model\UFPR-AMR Dataset\testing - kopie"
image_paths = [
...
for image_path in image_paths:
    ...
    if boxes is not None and len(boxes) > 0:
        ...
        if len(box_data) > 0:
            ...
            foutmelding = f"GEEN GESCHIKTE BOXEN"
            print(f"Afbeelding '{os.path.basename(image_path)}': {foutmelding}")
            
            with open("resultaten.csv", 'a', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow([foutmelding, foutmelding])
            
            continue
    else:
        foutmelding = f"GEEN OBJECTEN GEDETECTEERD"
        print(f"Afbeelding '{os.path.basename(image_path)}': {foutmelding}")
    ...
    for i in range(n_digits):
        ...
        if slechte_match:
            ...
            # Anders: goede voorspelling
            # Zoek bij goede cijfers op index i
            # Let op: goede_cijfers heeft niet per se expliciete indexen
            # We reconstrueren via volgorde
    ...
    def preprocess_image(img, idx=0, image_path="onbekend", target_size=64, inner_size=60):
        ...
        if not rows.any() or not cols.any():
            foutmeldingen.append(f"Cijfer {idx + 1}: leeg binair beeld (image: {os.path.basename(image_path)})")
            print(f"⚠️  Cijfer {idx + 1}: leeg binair beeld – skeletonisatie overgeslagen.")
    ...
    class PyGSkeletonDataset(Dataset):
        ...
        def _image_to_graph(self, img, idx):
            skeleton = skeleton = preprocess_image(cijfer, idx=originele_index, image_path=image_path)
    ...
    for idx, cijfer in enumerate(slechte_cijfers):
        ...
        skeleton = preprocess_image(cijfer, idx=originele_index, image_path=image_path)
        G = skeleton_to_graph(skeleton)
        G_proj = project_graph_to_grid(G, grid_size=8)
        data = graph_to_pyg_data(G_proj, grid_size=8).to(device)

        # Check op geldige edge_index
        if data.edge_index.numel() == 0 or data.edge_index.ndim != 2:
            foutmeldingen.append(f"Cijfer {originele_index + 1}: lege of ongeldige edge_index (image: {os.path.basename(image_path)})")
            print(f"Cijfer {originele_index + 1}: ongeldige edge_index – reconstructie overgeslagen.")
    ...
    header = ["bestand", "zonder_reconstructie", "met_reconstructie"]
    ...
    with open(csv_path, 'a', newline='', encoding='utf-8') as f:
        ...
        writer.writerow([os.path.basename(image_path), zonder_reconstructie, met_reconstructie])

    print(f"Output: {cijfer_string}")


    # === Foutmeldingen loggen ===
    alle_foutmeldingen.extend(foutmeldingen)
    if foutmeldingen:
        fout_log_path = "foutmeldingen_log.txt"
        with open(fout_log_path, 'a', encoding='utf-8') as f:
            for melding in foutmeldingen:
                f.write(melding + "\n")

        print("\nFoutmeldingen tijdens verwerking:")
        for melding in foutmeldingen:
            print("-", melding)
